...
bool esValida(int d, int k, long int i) {//
	int dSig = d+1;
	long int limite = (pow(10, (k - 1)) * (dSig));//ponemos como limite la suiguiente cifra del primer digito,
...
bool esSolucion(int d, int k, long int i) {
	int sum=0;
	long int aux = i;
...
void sumaSumDiv(int d, int k, int& SolucionSum, long int& i) {
	if (esValida(d, k, i)) {
		if (esSolucion(d, k, i)) {//miramos si es sumDivisible el numero
			long int sinUltimoDig = i / 10;//quitamos el ultimo digito
...
int num_sumdivisibles(int d, int k) {
	...
	long int empiezaRecorrido = (pow(10, (k - 1)) *d);//Comenzamos con el primer numero con digito D, que tenga k digitos
	int suma = 0;
