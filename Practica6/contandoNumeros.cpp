...
void cuenta_sinrepetidos_menoresque(t_num n,
	...
	// A implementar
	if (k < d) {
		t_num numerosK_esimos = num_menos_num_digs * (10 - k);
		k++;
		cuenta_sinrepetidos_menoresque(n, num_menos_num_digs, num_mismo_num_digs, k, d);
		num_menos_num_digs += numerosK_esimos;
	}
	//B implementar
	//caso base de B
	else if (n == 1) {
	...
	//cantidad de numeros distintos con d-digitos
	// B = B0 + B1 donde:
	//B0 = B’ * (10 – D’), donde D’ es el número de dígitos de n’ 
	//Si en n’ no hay dígitos repetidos, B1 es la cantidad de números de la
	//forma n’d’, tal que d’ < d y además d’ no aparece en n’.En otro caso,B1 será 0.
	else {
...
t_num num_sinrepetidos_menoresque(t_num n) {
	...
	if (n == 0)
		...
		t_num a;// Ck+1 = Ck * (10 – k)
		//donde A = C1 + C2 + … + CD-1, con D el número de dígitos de n. 
		t_num b;//Si n es un único dígito, B = n-1
		t_num num = n;
		int k = 0;
		int d = 0;
			while (num / 10 > 0){
		...
		cuenta_sinrepetidos_menoresque(n,a,b,k,d);
		return (a + b + 1);
	}
